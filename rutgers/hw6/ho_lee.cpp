...
double WINAPI
xll_VarlogD(double u, double v, double sigma)
{
#pragma XLLEXPORT
	double result(std::numeric_limits<double>::quiet_NaN());

	try {
		result = ho_lee::VarlogD(u, v, sigma);
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
	}
...
// Construct HO.LEE.SWAPTION.FORWARD.VALUE!!!
// static AddInX xai_ho_lee_swaption_forward_value(...
// The data type for xfp is XLL_FPX.
double WINAPI
xll_ho_lee_swaption_forward_value(double sigma, double c, xfp* pt, xfp* pD)
{
#pragma XLLEXPORT
	double result(std::numeric_limits<double>::quiet_NaN());

	try {
		ensure (size(*pt) == size(*pD));
		// Fill in the appropriate arguments!!!
		result = ho_lee::swaption_forward_value(0, 0, 0, 0, 0);
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
	}
